...
- `src/errors.ts` — error kernel: APIError, ErrorHandler, createErrorHandler, fail/errors/httpErrors (zero-dep leaf; ADR-005/011)
- `src/validation.ts` — coercion + validator: ArkType, coerceDeep/coerceValue, resolveRef, is*Type guards, arktypeValidator (depends on errors.ts + arktype; ADR-011 step 3)
- `src/registry.ts` — schema registry/hashing: sha1Hex, rewriteRefs, schemaCache, schemaToOA, getErrorSchemaRef (pure over a structural SchemaHost; ADR-011 step 4)
- `src/spec.ts` — OpenAPI document model + emission: AuthScheme/SecurityScheme/OAuth2Flows, RouteConfig/RouteHandler/StoredRoute, buildSpec/buildResponses/addObjectParams, resolveSuccessCode (pure over routes + ComponentRegistry; ADR-011 step 5)
- `src/openapi.ts` — orchestrator: OpenAPIHono class (default `onError` + `notFound`), dispatch, doc()/registerSecurityScheme; re-exports ArkType, arktypeValidator, AuthScheme/SecurityScheme/OAuth2Flows, RouteConfig + paths helpers for barrel stability
- `src/api.ts` — library: createApi, api, auth, docs, fail (re-exports APIError from errors.ts)
...
- Built-in auth strategies (opt-in, ADR-012/013/014): `auth.session(name, opts)`, `auth.jwt(name, opts)`, `auth.oauth(name, opts)` (and `auth.strategy(name, { type, ...opts })`) register guards through the same path as `auth(name, mw, scheme?)`, so `{ auth: name }` keeps the 401 + `security` + `securitySchemes` behavior. Session = signed `sid.hmac` cookie + pluggable `SessionStore` — cookie is `Secure` by default (dev-over-http opt-out via `cookie: { secure: false }`), `hostPrefix` renames to `__Host-sid` (which forces `Secure` + `Path=/` + no `Domain`, so it is **not** combined with a non-`/` `path`), and CSRF defaults to `origin` (a cross-site mutating request is rejected 403; `"origin"` mode **requires the `origin` option** — the strategy throws if unset). JWT = `jose` access tokens (HS256 default; opt-in `keys`/`kid` rotation, `jwks`, asymmetric RS256/EdDSA, `algorithms` alg-pinning, `refreshTransport` HttpOnly refresh cookie, `generateKey()` to build a ready-to-wire asymmetric keypair) + opaque rotating/revocable refresh tokens. OAuth = Google authorization-code + PKCE flow (PKCE on by default; state cookie `Secure`; provider `error` → `onError`). **OAuth registers only the `oauth2` scheme + mounts `/start`/`/callback`; it is NOT a request guard** — protect downstream routes with a jwt/session gate. The `/start`+`/callback` flow routes are intentionally NOT emitted in the OpenAPI `paths` (the spec emitter is JSON-body-oriented; a `302` redirect would mismatch), but the `oauth2` securitySchemes entry is. Stores default in-memory — supply a durable `SessionStore`/`RefreshTokenStore` in prod (ponytail). **Opt-in password helper:** `import { hashPassword, verifyPassword } from "peta-hono/password"` — `@noble/hashes` scrypt, self-describing hash, constant-time verify; credential hashing only. **Input vs emitted security-scheme type:** `AuthScheme` is the narrow `auth(name, mw, scheme?)` input (http bearer/basic, apiKey header/query — unchanged since v0.5.4); `SecurityScheme` is the wide emitted `components.securitySchemes` type (adds `apiKey` `in: "cookie"` and `oauth2` with `authorizationCode` flows). `arctic`/`oslo` are deprecated — the hand-rolled OAuth flow is deliberate.
- Cookie helpers are public re-exports: `createCookieTransport` / `cookieNameFor` (+ the `CookieTransport` / `CookieSerializeOptions` types), used to read/set/clear an opaque bearer token in an HttpOnly cookie (e.g. JWT `refreshTransport`). `CookieTransport` defaults to HttpOnly + Secure + SameSite=Lax, path-scoped, with **`hostPrefix: false`** (a deliberate default so `path` scoping works); `hostPrefix: true` renames `__Host-<name>` and **forces `Path=/`**.
- All errors — handler-thrown `APIError`, validator failures, unexpected throws — route through `app.onError` (single chokepoint). `OpenAPIHono` registers a default `onError`; `createApi()` overrides it with its own policy. The `OpenAPIHono` ctor also registers `this.notFound(...)` so an unmatched route returns `application/json {error:"Not Found"}` via the shared `createErrorHandler` policy — unifying the 404 shape with `fail.notFound()`. (Error policy lives in `src/errors.ts`; validator throws `APIError(400)` from `src/validation.ts` — no circular import, ADR-011.)
...
- Default `docs()` is unauthenticated (`ponytail: no auth on docs — protect it in production if needed` in `examples/basic/routes.ts`). For private APIs, opt in with `docs({ auth })` — a raw Hono `MiddlewareHandler`, or a registered auth name (e.g. `auth('session', ...)` → `docs({ auth: 'session' })`). It applies the guard to both the spec path and the UI path (glob, so `/docs` **and** any Scalar sub-path are covered) and registers it **before** mounting — Hono matches in registration order, so `app.use` after `app.get` would NOT guard. Unauthenticated default is unchanged/non-breaking; an unregistered auth name throws (mirrors `api()`'s guard). The equivalent manual recipe still works:
  ...
  // docs()
  ```
- Header schemas must use lowercase keys — Hono's `c.req.header()` lowercases via Fetch `Headers`; `addObjectParams` (src/spec.ts) emits `name.toLowerCase()` when `in === "header"` so spec and runtime match. Declare `type({ "x-api-key": "string" })` not `"X-Api-Key"` (ponytail: ceiled — `coerceDeep` does not auto-lowercase; strict fail-fast 400 if you use uppercase).
...
- Module-layering invariant (ADR-011): `errors ← validation ← registry ← spec ← openapi ← api ← index` — never let a lower module import a higher one; `registry.ts` takes a structural `SchemaHost` (`{ schemas: Map }`) instead of importing `ComponentRegistry` from spec.ts, and `spec.ts` never imports `openapi.ts`.
