...
- **Shorthands:** `api.get/post/put/patch/delete/del(path, configWithoutMethodPath, handler)` via `makeMethodHelper(M): ApiMethodHelper<Auth,E>` delegating to `api({method: M, path, ...config}, handler)`. `ApiMethodHelper<Auth,E>` is an explicit two-overload interface (not `ReturnType<typeof makeMethodHelper>`) — preserves `auth?:undefined` vs `auth:string` → `AuthField<Auth>` distinction. Fixes grilling Q6 where `ReturnType` collapsed to `auth?:string` / `req & {auth:Auth}` and let `api.get("/x", {auth:"required"}, ({auth})=>...)` on `createApi<undefined>` pass without error while classic `api({method,path,auth})` still errored. `api.delete` + alias `api.del` (Hono `del` compat). Classic form unchanged.
...
- **Gaps (grilling Q6–11):** ~~`makeMethodHelper` uses `api as any` casts hiding overload launder; `ReturnType<typeof makeMethodHelper>` collapses two overloads → negative `api.get` with `auth` on `createApi<undefined>` not type-error (classic `api()` still errors). Future tighten: explicit overload types not `ReturnType`, or drop `string & {}` fallback.~~ **Fixed 2026-08-26:** `ApiMethodHelper<Auth,E>` replaces `ReturnType`; both `auth?:undefined` and `auth:string` overloads are now preserved for shorthands, so `createApi<undefined>` → `api.get("/x", {auth:"required"}, ({auth})=>...)` is a type error via `ReqFor & AuthField<undefined>=ReqFor` (missing `auth`), matching classic form. Remaining `api as unknown` cast is internal impl-only (one handler cast for `Auth` distribution). Optional path spec `required:false` with `/{id}` path is technically invalid OAPI 3.0 (`required` should be true) — options: emit two paths or keep `required:false` + doc note (still open).
