...
import { isObjectSchema } from "./validation.js";
// --- Success-code policy ---
/**
 ...
 */
export function resolveSuccessCode(status, responseKeys) {
    return (status?.toString() ?? responseKeys.find((k) => k.startsWith("2") || k.startsWith("3")) ?? "200");
}
// --- Spec emission ---
/** Emit the OpenAPI 3.0 document for all registered routes. */
export async function buildSpec(routes, components, config) {
    const paths = {};
 ...
 */
async function buildResponses(config, components) {
    const responses = {};
...
async function addObjectParams(params, schema, inLocation, components) {
