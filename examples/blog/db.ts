...
// Create tables
await client.execute("CREATE TABLE IF NOT EXISTS posts (id TEXT PRIMARY KEY, title TEXT NOT NULL, content TEXT NOT NULL, author_id TEXT NOT NULL, created_at TEXT NOT NULL)");
await client.execute("CREATE TABLE IF NOT EXISTS comments (id TEXT PRIMARY KEY, post_id TEXT NOT NULL, content TEXT NOT NULL, author_id TEXT NOT NULL, created_at TEXT NOT NULL)");
...
export async function listPosts(limit: number, offset: number): Promise<{ posts: Post[]; total: number }> {
	const rows = await db
		.select()
		.from(posts)
...
export async function createComment(postId: string, content: string, authorId: string): Promise<Comment | undefined> {
	// Verify post exists
