...
- **Built-in auth strategies** — session (cookie), JWT (bearer access + rotating refresh), and Google OAuth2 (authorization-code + PKCE), all opt-in and composable with the existing `{ auth: name }` gating. JWT uses `jose` (with opt-in `keys`/`jwks` rotation + `algorithms` alg-pinning); the shared crypto is `@noble/hashes`. Emit the matching OpenAPI `securitySchemes` (`apiKey/in:cookie`, `bearer`, `oauth2/authorizationCode`). See the strategies example: `nub examples/strategies/index.ts`.
